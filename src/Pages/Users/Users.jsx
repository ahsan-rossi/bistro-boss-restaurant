...
import { FaRegTrashCan, FaTrashCan, FaUser } from "react-icons/fa6";
...
const Users = () => {
  ...
  const handleUserRole = (id, name, role) => {
    const newRole = role === "admin" ? "user" : "admin"; // Toggle role
    Swal.fire({
      title: "Are you sure?",
      text: `You are about to make ${name} an ${newRole}.`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
      ...
      if (result.isConfirmed) {
        axiosSecure
          .patch(`/users/admin/${id}`, { role: newRole })
          .then((res) => {
            if (res.data.modifiedCount > 0) {
              Swal.fire({
                title: "Role Updated!",
                text: `${name} is now an ${newRole}.`,
                icon: "success",
                timer: 1500,
                showConfirmButton: false,
              });
          ...
          <div className="md:hidden space-y-4">
            {users.map((user, index) => (
              ...
              >
                ...
                <div>
                  <button
                    className={`btn btn-ghost rounded-md p-3 ${user.role === "admin" ? "bg-[#B58130] hover:bg-[#835D23] text-yellow-100" 
                      : "bg-gray-300 hover:bg-gray-400 text-gray-900"}`}
                    onClick={() =>
                      handleUserRole(user._id, user.name, user.role)
                    }
                  >
                    <FaUser></FaUser>
                  </button>
